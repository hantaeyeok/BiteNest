...
-- COMMENT_LIKE_TB 테이블의 USER_NO 외래 키 제약 조건 제거
ALTER TABLE COMMENT_LIKE_TB
DROP CONSTRAINT FK_USER_LIKE;

-- RECIPE_COMMENT_TB 테이블의 USER_NO 외래 키 제약 조건 제거 (필요한 경우)
...
-- 댓글 테이블 생성
CREATE TABLE RECIPE_COMMENT_TB (
    COMMENT_CD       NUMBER           NOT NULL, -- 댓글 기본키
    RECIPE_CD        NUMBER           NOT NULL, -- 레시피 기본키
    USER_NO          NUMBER           NOT NULL, -- 작성자 유저 번호
    PARENT_COMMENT_CD NUMBER           NULL,     -- 부모 댓글 번호 (대댓글일 경우 부모 댓글 번호)
    COMMENT_TEXT     VARCHAR2(255)    NOT NULL, -- 댓글 내용
    IMAGE_URL        VARCHAR2(225)    NULL,     -- 이미지 URL (첨부 이미지 경로)
    CREATED_DT       TIMESTAMP        NOT NULL, -- 작성일
    UPDATED_DT       TIMESTAMP        NULL,     -- 수정일
    CONSTRAINT PK_RECIPE_COMMENT PRIMARY KEY (COMMENT_CD) -- 기본키 제약조건
);
...
-- 댓글 좋아요 테이블 생성
CREATE TABLE COMMENT_LIKE_TB (
    LIKE_CD          NUMBER           NOT NULL, -- 좋아요 기본키
    COMMENT_CD       NUMBER           NOT NULL, -- 댓글 기본키 (참조)
    USER_NO          NUMBER           NOT NULL, -- 유저 번호
    LIKED_DT         TIMESTAMP        NOT NULL, -- 좋아요한 일시
    CONSTRAINT PK_COMMENT_LIKE PRIMARY KEY (LIKE_CD), -- 기본키 제약조건
    CONSTRAINT FK_COMMENT_LIKE_COMMENT FOREIGN KEY (COMMENT_CD) REFERENCES RECIPE_COMMENT_TB (COMMENT_CD) ON DELETE CASCADE -- 댓글 테이블 외래키 제약조건
);
...
CREATE TABLE RECIPE_BOOKMARK_TB (
    BOOKMARK_CD NUMBER NOT NULL,            -- 북마크 기본키
    RECIPE_CD   NUMBER NOT NULL,            -- 레시피 기본키 (참조)
    USER_ID     VARCHAR2(30) NOT NULL,      -- 유저 ID
    CREATED_DT  TIMESTAMP DEFAULT SYSDATE,  -- 생성일시
...
INCREMENT BY 1        -- 값이 증가할 단위 (1씩 증가)
NOCACHE               -- 미리 시퀀스 값을 캐시하지 않음
NOCYCLE; 
